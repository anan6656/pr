...
# ====================================================================
# 修复 NumPy 兼容性问题：禁用 X86_V2 优化
# ====================================================================
# 解决错误：NumPy was built with baseline optimizations: (X86_V2) but your machine doesn't support: (X86_V2)
# 注意：必须在导入 numpy 和其他科学计算库之前设置
try:
    ...
    # 标记已设置
    os.environ['_NUMPY_X86_V2_PATCHED'] = '1'
except Exception:
    pass  # 忽略设置失败
...
# --- 二值化与边缘 ---
THRESHOLD = 110    # 二值化阈值：灰度 > THRESHOLD 视为亮（柱子区域）。若固定阈值结果异常（全黑/全白），自动改用 Otsu
...
def calculate_image_quality(gray):
    """评估图像质量，返回质量评分和特征"""
    # 边缘强度
    edges = cv2.Sobel(gray, cv2.CV_32F, ksize=3, dx=1, dy=0)
    ...
    histogram_spread = len(histogram[histogram > 0])
...
def preprocess_image(image_path):
    ...
    - 优化流程：CLAHE 对比度增强 → 混合模糊 → 图像质量评估 → 自适应阈值策略 → 优化形态学
    """
    ...
    # === 优化 2: 混合模糊策略（中值 + 高斯）===
    # 先中值滤波去椒盐噪声，再高斯模糊
    blur_median = cv2.medianBlur(gray_enhanced, 3)
    blur = cv2.GaussianBlur(blur_median, (5, 5), 0)

    # === 优化 3: 图像质量评估 ===
    quality = calculate_image_quality(blur)
    
    # === 优化 4: 根据图像质量自动选择阈值策略 ===
    if quality['edge_strength'] > 50 and quality['contrast'] > 80:
        # 边缘清晰、对比度高 → 简单全局阈值 + Otsu 保护
    ...
    elif quality['edge_strength'] < 30 or quality['contrast'] < 40:
        # 边缘模糊、对比度低 → 混合全局和自适应阈值
        lower = max(0, THRESHOLD - THRESHOLD_TOL)
        _, binary_global = cv2.threshold(blur, lower, 255, cv2.THRESH_BINARY)
        ...
        # 融合两个结果：都认为是白色才保留（更严格）
        binary = cv2.bitwise_and(binary_global, binary_adaptive)
        print(f"[info] 图像质量较低 (边缘={quality['edge_strength']:.1f}, 对比度={quality['contrast']:.1f})，已启用混合阈值")
    else:
        # 中等质量 → 混合方法
        lower = max(0, THRESHOLD - THRESHOLD_TOL)
        _, binary_global = cv2.threshold(blur, lower, 255, cv2.THRESH_BINARY)
        _, binary_otsu = cv2.threshold(blur, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
        # 使用 Otsu 作为主要结果，全局阈值作为验证
        binary = binary_otsu
        print(f"[info] 使用 Otsu 自适应阈值 (边缘={quality['edge_strength']:.1f}, 对比度={quality['contrast']:.1f})")
    ...
    if num_labels > 1:
        ...
        if len(areas) > 0:
            ...
            # 面积越大，核越大
            kernel_size = max(3, min(21, int(np.sqrt(median_area) / 10)))
            kernel_size = kernel_size if kernel_size % 2 == 1 else kernel_size + 1
        else:
            kernel_size = MORPH_CLOSE_K if MORPH_CLOSE_K % 2 == 1 else MORPH_CLOSE_K + 1
    else:
        kernel_size = MORPH_CLOSE_K if MORPH_CLOSE_K % 2 == 1 else MORPH_CLOSE_K + 1
    ...
    binary_closed = cv2.morphologyEx(binary_opened, cv2.MORPH_CLOSE, kernel, iterations=2)
...
def find_valleys_optimized(profile, valley_ratio=VALLEY_RATIO, min_gap=20):
    """
    改进的谷底检测：多级策略 + 导数辅助 + 过近谷底合并
    ...
    # 修复：数据过小时返回空
    if n < 2:
        print(f"[warn] 投影曲线过短 (n={n})，无法检测谷底")
        return []
    
    # === 优化 1: 使用导数找更精确的谷底 ===
    # 注意：cv2.GaussianBlur 对 1D 数组返回 2D (1,N)，需要 reshape+flatten 保证 1D
    profile_smooth = cv2.GaussianBlur(profile.reshape(1, -1), (1, 11), 0).flatten()
    derivative = np.gradient(profile_smooth)
    
    maxv = np.max(profile_smooth)
    if maxv <= 0:
        return []
    
    # === 优化 2: 多级检测策略 ===
    
    # 第一级：找明显谷底（低于 0.2*max）+ 导数变号
    low = profile_smooth < 0.2 * maxv
    valleys_strict = []
    in_v = False
    start = 0
    for x in range(len(profile_smooth)):
        if low[x] and not in_v:
            ...
            in_v = False
            if x - start >= 3:
                valleys_strict.append((start, x - 1))
    if in_v and len(profile_smooth) - start >= 3:
        valleys_strict.append((start, len(profile_smooth) - 1))
    ...
    # 将导数谷值转换为区间
    valleys_derivative = []
    for v in derivative_valleys:
        # 查找谷值周围低于 0.3*max 的范围
        lo = hi = v
        while lo > 0 and profile_smooth[lo] < 0.3 * maxv:
            lo -= 1
        while hi < len(profile_smooth) - 1 and profile_smooth[hi] < 0.3 * maxv:
            hi += 1
        if hi - lo >= 3:
            valleys_derivative.append((lo, hi))
    
    # 合并两个来源的谷底
    all_valleys = valleys_strict + valleys_derivative
    
    # 第二级：如果没有找到足够的谷底，放宽阈值
    if len(all_valleys) < 2:
        low_relaxed = profile_smooth < 0.3 * maxv
        valleys_relaxed = []
        in_v = False
        start = 0
        for x in range(len(profile_smooth)):
            if low_relaxed[x] and not in_v:
                in_v = True
                start = x
            elif not low_relaxed[x] and in_v:
                in_v = False
                if x - start >= 3:
                    valleys_relaxed.append((start, x - 1))
        if in_v and len(profile_smooth) - start >= 3:
            valleys_relaxed.append((start, len(profile_smooth) - 1))
        all_valleys.extend(valleys_relaxed)
    ...
    # 按左边界排序
    all_valleys.sort(key=lambda v: v[0])
    
    merged = []
    for v in all_valleys:
        if merged and v[0] - merged[-1][1] < min_gap:
            # 合并（延长上一个谷底的右边界）
            merged[-1] = (merged[-1][0], max(merged[-1][1], v[1]))
        else:
            merged.append(v)
...
def trace_column_ranges(binary, left, right, y_top, y_bottom):
    ...
    for y in range(y_bottom, y_top - 1, -1):
        ...
        # === 优化：边缘检测辅助 ===
        # 计算当前行的边缘强度
        row_edges = cv2.Canny(row[np.newaxis, :], 50, 150)
        edge_positions = np.where(row_edges[0] > 0)[0]
        ...
        def score_segment(seg):
            ...
            # 加分：段内边缘点少（更平滑）
            edge_penalty = sum(1 for e in edge_positions if wl <= e <= wr)
            # 加权评分：重叠优先，但避免边缘噪声
            return overlap - 0.1 * edge_penalty
...
def draw_1d_profile_vertical(data, width, height, color=(255, 0, 0), fill_color=(200, 200, 255), title=""):
    ...
    # 修复：数据过小时返回空白图
    if n < 2:
        print(f"[warn] draw_1d_profile_vertical: 数据过短 (n={n})，返回空白图")
        canvas = np.full((height, width, 3), 255, dtype=np.uint8)
        if title:
            cv2.rectangle(canvas, (0, 0), (width - 1, 26), (200, 200, 200), -1)
            cv2.putText(canvas, title + " (无效数据)", (8, 18), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 0, 0), 2)
        return canvas
...
def draw_1d_profile_horizontal(data, width, height, color=(255, 0, 0), fill_color=(200, 255, 200), title=""):
    ...
    # 修复：数据过小时返回空白图
    if n < 2:
        print(f"[warn] draw_1d_profile_horizontal: 数据过短 (n={n})，返回空白图")
        canvas = np.full((height, width, 3), 255, dtype=np.uint8)
        if title:
            cv2.rectangle(canvas, (0, 0), (width - 1, 26), (200, 200, 200), -1)
            cv2.putText(canvas, title + " (无效数据)", (8, 18), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 0, 0), 2)
...
class App(tk.Tk):
    ...
    def __init__(self):
        ...
        self._line_profile_fig = None        # matplotlib 灰度曲线图
        self._line_profile_canvas = None     # matplotlib 画布
        self._line_profile_data = None       # 当前直线剖面数据
        self._line_profile_selected_x = None # 当前选中位置数据索引
        self._ang = []                       # 角度量测：三个点 (A 端点，顶点 V, C 端点)
    ...
    def _build_ui(self):
        ...
        # —— 原始图画布（框选/量测都在上面进行） + 灰度曲线图 ——
        self.frm_mid_orig = ttk.Frame(self.frm_mid)
        self.frm_mid_orig.grid(row=1, column=0, sticky="nsew", padx=2, pady=2)
        
        # 左侧：原始图画布
        self.frm_orig = ttk.LabelFrame(self.frm_mid_orig,
        ...
        if MATPLOTLIB_AVAILABLE:
            self.frm_line_profile = ttk.LabelFrame(self.frm_mid_orig, 
                                                   text="直线量测灰度剖面（点击曲线可查看灰度值）")
            self.frm_line_profile.pack(side="left", fill="both", expand=True, padx=(2, 0))
            # matplotlib 图表
            self._line_profile_fig = Figure(figsize=(4, 2), dpi=100)
            self._line_profile_ax = self._line_profile_fig.add_subplot(111)
            self._line_profile_ax.set_xlabel('像素索引')
            self._line_profile_ax.set_ylabel('灰度值')
            self._line_profile_ax.set_title('')
            self._line_profile_ax.grid(True, alpha=0.3)
            self._line_profile_canvas = FigureCanvasTkAgg(self._line_profile_fig, 
                                                          master=self.frm_line_profile)
            self._line_profile_canvas.get_tk_widget().pack(fill="both", expand=True)
            self._line_profile_canvas.mpl_connect('button_press_event', 
    ...
    def _draw_dist(self):
        ...
        if self._m2 is not None:
            ...
            # 计算并显示灰度剖面
            if MATPLOTLIB_AVAILABLE:
                self._compute_and_draw_profile(ax, ay, bx, by)
    ...
    def _update_canvas_info(self):
        ...
        if t == "dist":
            ...
            if res is not None:
                ...
                self.lbl_roi_info.configure(
                    text="提示：角度量测按 A 端点 → 顶点 V → C 端点 依次单击三个点",
                    foreground="#555555")
        else:
            self.lbl_roi_info.configure(
                text="提示：先在上方选择工具（框选区域/直线量测/角度量测）",
                foreground="#555555")
        
    # ===================================================================
    # 直线量测灰度剖面图功能
    # ===================================================================
    def _compute_and_draw_profile(self, ax, ay, bx, by):
        """计算直线量测路径上的灰度剖面并绘制"""
        if self._orig_bgr is None:
            return
        ...
        for t in range(int(length) + 1):
            x = int(ax + (bx - ax) * t / max(length, 1))
            y = int(ay + (by - ay) * t / max(length, 1))
            if 0 <= y < self._oh and 0 <= x < self._ow:
                # 取 BGR 的 B 通道（灰度）
                points.append(self._orig_bgr[y, x, 0])
            else:
                points.append(0)
            
        self._line_profile_data = np.array(points, dtype=np.float32)
            
        # 绘制灰度剖面图
        self._draw_line_profile()
        
    def _draw_line_profile(self):
        """在 matplotlib 图表上绘制灰度剖面"""
        if self._line_profile_data is None or len(self._line_profile_data) < 2:
            return
            
        # 清空图表
        self._line_profile_ax.clear()
        data = self._line_profile_data
        max_val = data.max() if data.max() > 0 else 1
            
        # 绘制曲线
        x_vals = np.arange(len(data))
        self._line_profile_ax.plot(x_vals, data, color='#0066cc', linewidth=1.5,
                                   marker='.', markersize=3, alpha=0.7)
            
        # 添加选中的高亮线
        if self._line_profile_selected_x is not None:
            self._line_profile_ax.axvline(x=self._line_profile_selected_x, 
                                          color='#ff0000', linestyle='--', 
                                          linewidth=1, alpha=0.8)
            # 显示该点的灰度值
            idx = int(np.clip(self._line_profile_selected_x, 0, len(data) - 1))
            gray_val = data[idx]
            self._line_profile_ax.text(idx, gray_val, f"\u2193{gray_val:.1f}",
                                       color='#ff0000', fontsize=9,
                                       ha='center', va='bottom')
            
        # 设置标签和标题
        self._line_profile_ax.set_xlabel('像素索引', fontsize=9)
        self._line_profile_ax.set_ylabel('灰度值', fontsize=9)
        self._line_profile_ax.set_title(f'灰度值：0~{int(max_val)}', fontsize=10)
        self._line_profile_ax.grid(True, alpha=0.3)
    ...
    def _on_profile_click(self, event):
        """处理 matplotlib 图表点击事件"""
        if not MATPLOTLIB_AVAILABLE or self._line_profile_data is None:
            return
            
        # 检查是否在图表区域内
        if event.inaxes != self._line_profile_ax:
            return
            
        # 获取点击位置对应的数据索引
        idx = int(event.xdata) if event.xdata is not None else None
        if idx is None:
            return
            
        # 限制索引范围
        idx = int(np.clip(idx, 0, len(self._line_profile_data) - 1))
        self._line_profile_selected_x = idx
    ...
    def _highlight_selected_point(self):
        """在原图上高亮显示选中的灰度剖面点"""
        if self._m1 is None or self._m2 is None or self._line_profile_data is None:
            return
            
        idx = self._line_profile_selected_x
        ax, ay = self._m1
        bx, by = self._m2
        length = math.hypot(bx - ax, by - ay)
        ...
        if length > 0:
            # 计算选中点对应的图像坐标
            tx = ax + (bx - ax) * idx / length
            ty = ay + (by - ay) * idx / length
            ...
            # 高亮选中点
            cx, cy = self._orig_to_cv(tx, ty)
            r = 8
            self.cv_orig.create_oval(cx - r, cy - r, cx + r, cy + r,
                                     outline="#ff0000", fill="#ff3333", width=2, tags="ovl")
                
            # 显示灰度值
            gray_val = self._line_profile_data[idx]
            self.cv_orig.create_text(cx, cy - 15,
                                     text=f"{gray_val:.1f}",
                                     fill="#ff0000",
                                     font=("Microsoft YaHei UI", 9, "bold"),
                                     tags="ovl")
