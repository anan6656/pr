...
def _detect_columns_geometric(gray, roi_mode=False):
    ...
    if GEOMETRIC_DETECTION_ENABLE:
        ...
        for variant_name, variant_gray in variants[:max_attempts]:
            ...
            candidate_columns, candidate_profile = _detect_columns_geometric(
                variant_gray, roi_mode=roi_mode
            )
...
def profile_overlap_score(w, target_cd):
    """计算传统 IoU 面积重叠率（仅作诊断，范围 0~100）。

    Judge 主评分使用 :func:`profile_target_ratio_score`，因为 IoU 会把
    过宽和过窄都压到 100 以下，无法表达用户关心的“Profile 比 Target
    大”。保留这个辅助指标便于旧脚本或调试时比较形状重叠程度。
    """
    if w is None or target_cd is None or len(w) < 5:
        return None
    try:
    ...
    if len(arr) < 5:
        return None
    arr = np.maximum(arr, 0.0)
    overlap = float(np.sum(np.minimum(arr, target)))
    union = float(np.sum(np.maximum(arr, target)))
    if union <= 1e-12:
        return None
    return 100.0 * overlap / union


def profile_iou_score(w, target_cd):
    """兼容旧调用名，返回新的方向性 Profile/Target (%) 评分。

    旧版本把这个函数用于 IoU×100。为了让已有调用自动得到用户期望
    的 >100 结果，名称保留但语义改为 :func:`profile_target_ratio_score`；
    真正的 IoU 诊断请调用 :func:`profile_overlap_score`。
    """
    return profile_target_ratio_score(w, target_cd)
...
def recompute_stats_and_target(payload):
    """不重新做图像检测，只按当前比例尺/形貌/Target 参数重算统计与判定。"""
    nm_per_px = SCALE_NM / SCALE_PIXELS
    data = analyze_columns(payload["columns"], nm_per_px)
    meta = payload.get("detection")
    if not isinstance(meta, dict):
        meta = _detection_meta_from_aux(payload, payload.get("columns"))
    ...
    data["target"] = build_target_report(data)
    if isinstance(data.get("detection"), dict):
        data["target"]["detection"] = data["detection"]
    payload["stats"] = data
    return data
...
def write_stats_file(out_txt_path, data, detection_meta=None):
    ...
    # 误以为它是限制在 0~100 的“相似度”。这些行不影响旧版
    # read_stats_file：该读取器只解析首行和 STAT_ROWS。
...
def parse_boundaries_table(txt_path):
    ...
    for line in lines[1:]:
        ...
        for i, lb in enumerate(labels):
            ...
            try:
                a, b = seg[1:-1].split(",", 1)
                data[lb].append((y, float(a), float(b)))
            except Exception:
                pass
...
def analyze_boundaries_file(txt_path, out_txt_path=None):
    """兼容旧文件/手工导入；程序自身新结果优先 analyze_columns()."""
    parsed = parse_boundaries_table(txt_path)
    labels = []
    per_col = []

    for lb, rows in parsed["columns"]:
        if not rows:
            continue
        labels.append(lb)
        per_col.append(analyze_pr_column(rows))
    ...
    if out_txt_path:
        write_stats_file(out_txt_path, data)
...
def read_stats_file(stats_path):
    """
    读取本程序生成的对齐统计表。
    若读取失败由 GUI 回退到 boundaries_nm 重新分析。
    """
    ...
    # 新格式首行是“统计项 / 全部柱平均 / PR1…PRn”。旧实现把
    # 平均列和最后一根柱混在一起，导致回读结果整体错位；这里按表头
    # 明确拆分，且兼容没有“全部柱平均”列的旧文本。
    if len(header) >= 2 and header[1] == "全部柱平均":
        cols = header[2:]
        has_average = True
    else:
        cols = header[1:]
        has_average = False
    descs = [d for _, d in STAT_ROWS]
    rows = []
    means = []
    ...
    for desc in descs:
        ...
        rest = found[len(desc):].split()
        need = len(cols) + (1 if has_average else 0)
        if len(rest) < need:
            return None

        vals = []
        offset = 1 if has_average else 0
        for s in rest[offset:offset + len(cols)]:
            vals.append(None if s == "-" else float(s))

        if has_average:
            m = rest[0]
            mean = None if m == "-" else float(m)
        else:
            mean = _mean_finite(vals)
...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        result_row.pack(fill="both", expand=True, pady=(4, 0))
    ...
    def _redraw_preview(self):
        ...
        self._put_preview("tool")
        self._put_preview("user")
        self._redraw_marks()
    ...
    def _put_preview(self, kind):
        ...
        if bgr is None:
            label.configure(image="", text="（尚无结果）")
            return
    ...
    def _profile_xy_to_index(self, xy):
        """把图像坐标投影回当前曲线索引，用于双向同步。"""
        if xy is None or self._line_profile_data is None:
            return None
        try:
            x, y = float(xy[0]), float(xy[1])
            n = len(self._line_profile_data)
            mode = (self._profile_meta or {}).get("mode")
            if n <= 1:
                return 0
            if mode == "row":
                value = x
                ...
                a = self._profile_meta.get("a")
                b = self._profile_meta.get("b")
                if not a or not b:
                    return None
                ...
                if den <= 1e-12:
                    return 0
                value = ((x - float(a[0])) * dx + (y - float(a[1])) * dy) / den
                value = min(max(value, 0.0), 1.0) * (n - 1)
                return int(min(max(round(value), 0), n - 1))
            else:
                return None
            return int(min(max(round(value), 0), n - 1))
        except Exception:
