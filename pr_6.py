...
运行: python pr_4.2.py
离线打包: python pr_4.2.py --build-exe
...
def draw_baseline_figure(report, figure=None, sample_id=None):
    """总览用编号关联样本；筛选时颜色、统计口径与总览保持一致。"""
    ...
    deltas = np.array([[s["metrics"].get(k) - ref["metrics"].get(k)
                        if _finite_number(s["metrics"].get(k)) and _finite_number(ref["metrics"].get(k))
                        else np.nan for k, _ in MORPH_SHAPES] for s in samples], dtype=float)
    labels = [label for _, label in MORPH_SHAPES]
    if len(samples) <= 1:
        spacing = .72 / max(1, len(samples))
        finite = deltas[np.isfinite(deltas)]
        extent = max(1., float(np.max(np.abs(finite))) if finite.size else 1.)
        for j in range(len(labels)):
            if j % 2 == 0:
                changes.axhspan(j-.5, j+.5, color="#edf2f7", zorder=0)
        for i, sample in enumerate(samples):
            color = colors(sample.get("color_index", 0) % 10)
            for j, delta in enumerate(deltas[i]):
                y = j + (i-(len(samples)-1)/2)*spacing
                if not math.isfinite(delta):
                    changes.text(extent*.03, y, f"{sample['sample_id']}  —", va="center", fontsize=8, color="#64748b")
                    continue
                changes.barh(y, delta, height=spacing*.55, color=color, alpha=.8, zorder=3)
                changes.plot(delta, y, "o", color=color, markersize=3, zorder=4)
                changes.annotate(f"{sample['sample_id']} {delta:+.2f}", (delta, y),
                                 xytext=(5 if delta >= 0 else -5, 0), textcoords="offset points",
                                 ha="left" if delta >= 0 else "right", va="center", fontsize=8,
                                 color="#b45309" if delta > 1e-8 else "#1d4ed8" if delta < -1e-8 else "#64748b")
        changes.axvline(0, color="#64748b", linewidth=1, zorder=2)
        changes.set_xlim(-extent*1.6, extent*1.6)
        changes.set_yticks(range(len(labels)), labels)
        changes.set_ylim(len(labels)-.5, -.5)
        changes.set_xlabel("Δ (nm)   ← 减少  |  增加 →")
        changes.grid(axis="x", color="#cbd5e1", alpha=.4)
    else:
        # 多样本采用带数字的矩阵，避免同一指标上的线段与数值重叠。
        matrix = deltas.T
        finite = matrix[np.isfinite(matrix)]
        extent = max(.01, float(np.max(np.abs(finite))) if finite.size else .01)
        changes.imshow(np.ma.masked_invalid(matrix), cmap="RdBu_r", vmin=-extent, vmax=extent, aspect="auto")
        changes.set_xticks(range(len(samples)), [s["sample_id"] for s in samples])
        changes.set_yticks(range(len(labels)), labels)
        for j in range(len(labels)):
            for i in range(len(samples)):
                value = matrix[j, i]
                changes.text(i, j, f"{value:+.2f}" if np.isfinite(value) else "—", ha="center", va="center",
                             fontsize=max(6, 9-len(samples)//6),
                             color="white" if np.isfinite(value) and abs(value) > .55*extent else "#253449")
        changes.set_xlabel("样本编号 · 红色增加 / 蓝色减少 / 白色接近 0")
        changes.set_xticks(np.arange(-.5, len(samples), 1), minor=True)
        changes.set_yticks(np.arange(-.5, len(labels), 1), minor=True)
        changes.grid(which="minor", color="white", linewidth=2)
        changes.tick_params(which="minor", bottom=False, left=False)
    changes.set_title("形貌增减 · Δ = 样本 − B0 (nm)", fontsize=11, fontweight="bold", color="#20334a", pad=14)
...
def draw_baseline_delta_bars(report, figure=None, sample_id=None):
    """独立的形貌增减横向点柱图；与右侧矩阵使用同一批 Δ 数据。"""
    if not MATPLOTLIB_AVAILABLE:
        raise RuntimeError("Baseline 轮廓图需要 Matplotlib")
    samples=[x for x in report["samples"] if sample_id is None or x["sample_id"]==sample_id]
    ref=report["baseline"]
    keys=[key for key,_ in MORPH_SHAPES]
    labels=[label for _,label in MORPH_SHAPES]
    values=np.array([[x["metrics"].get(k)-ref["metrics"].get(k)
        if _finite_number(x["metrics"].get(k)) and _finite_number(ref["metrics"].get(k)) else np.nan
        for k in keys] for x in samples],dtype=float)
    ...
    for i,sample in enumerate(samples):
        ...
        for j,value in enumerate(values[i]):
            ...
            if not math.isfinite(value):
                ax.text(-extent*.96,y,f"{sample['sample_id']}  —",va="center",ha="left",fontsize=8,color="#64748b")
                continue
    ...
    ax.set_title("形貌增减 · 横向点柱图",fontsize=13,fontweight="bold",color="#20334a",pad=14)
...
class BaselineWindow(tk.Toplevel):
    ...
    def __init__(self, app):
        ...
        bar_frame = ttk.Frame(panes)
        panes.add(bar_frame, text="形貌点柱图")
        self.bar_figure = self.bar_canvas = None
        if MATPLOTLIB_AVAILABLE:
            self.bar_figure = Figure(figsize=(10, 5.5), dpi=100)
            self.bar_canvas = FigureCanvasTkAgg(self.bar_figure, master=bar_frame)
    ...
    def _refresh_result_views(self, _event=None):
        ...
        if self.figure is not None:
            ...
            draw_baseline_figure(self.report, self.figure, sample_id=sample_id)
            self.canvas.draw_idle()
        if self.bar_figure is not None:
            sample_id = samples[0]["sample_id"] if self.result_sample_combo.current() > 0 else None
            draw_baseline_delta_bars(self.report, self.bar_figure, sample_id=sample_id)
            self.bar_canvas.draw_idle()
